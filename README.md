...
wxjtcyorgcontroller存在sql注入漏洞，在登录泛微云桥之后，可以借助该漏洞执行sql注入语句。
...
wxjtcyorgcontroller.exportOrg调用wxJTCYOrgInfoService.exportOrg函数的方式是这样的：
...
public void exportOrg() {
    try {
        ...
        if (StrKit.isBlank(syscorpid)) {
            ...
            this.renderFile(this.wxJTCYOrgInfoService.exportOrg(syscorpid, depts, users, fields));
        }
